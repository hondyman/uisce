...
func main() {
	...
	db.QueryRow("SELECT count(*) FROM tenants WHERE id = '99e99e99-99e9-49e9-89e9-99e99e99e999'").Scan(&count)
	fmt.Printf("Northwinds tenant exists: %d\n", count)

	db.QueryRow("SELECT count(*) FROM tenants WHERE gold_copy = true").Scan(&count)
	fmt.Printf("Gold copy tenants: %d\n", count)
