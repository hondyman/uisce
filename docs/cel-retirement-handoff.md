...
**Authoring-surface inventory closed**: `BPDesignerPage.tsx` right-panel configuration exposes no raw text fields for `delayExpr` or `entryCondition`. `durationHours` is a number input; `conditionLogic` is a structured `ConditionBranch` object. The backend (`designer.go`) derives expression strings from these structured fields — it does not accept arbitrary raw strings from the UI. Time-aware expressions (`now()`, `daysSince`, date arithmetic) are **impossible by construction** in the current BP designer. The time-function question is N/A by design; no shared-with-RDL FunctionSpec addition is required for the BP surface.
