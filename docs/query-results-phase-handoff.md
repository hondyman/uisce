...
**CI log evidence.** The Build Frontend red check on `35771443877.log:4333` is a `pnpm install` lockfile-spec mismatch — `specifiers in the lockfile (...) don't match specs in package.json (...)` — listing ~100 packages. Both lists are dumps of full spec maps, not isolated per-package errors. `grep -o '"tailwind-merge"[^,}]*'` strips context, so it cannot establish which parenthetical the match sits in; the user observed (and I now accept) that the visible log text labels the dump containing `"date-fns":"^3.6.0","tailwind-merge":"^3.6.0"` as **the lockfile specifier list**, not the package.json one. So the claim "`tailwind-merge` is `^3.6.0` in the lockfile" is supported by the CI log; the claim "`tailwind-merge` is `^3.3.1` in the current local lockfile" is supported by `grep frontend/package-lock.json` (line 244) — both can be true if the lockfile was bumped at one point and the bump didn't make it into the captured CI state, or if there's a separate install-state divergence. `ls frontend/*lock*` shows only `package-lock.json`; the repo root has `pnpm-lock.yaml` but `grep tailwind-merge pnpm-lock.yaml` returns empty, so the pnpm lockfile doesn't carry `tailwind-merge` at all (consistent with `tailwind-merge` being a frontend-only concern). The CI attribution method (`grep QueryResultsPanel|SavedQueryEditor|query-execution`) cannot catch undeclared-dep failures by construction — those failures don't mention any of this PR's filenames.
