...
**Sweeper row-mutation rule:** the TTL sweeper (Writer 4 in §4) queries for rows where `expires_at < NOW()` (TTL already reached), purges the MinIO artifact, emits an `EXPIRED` event, then **hard-deletes the row** (`DELETE WHERE id = $1`). The sweeper does NOT set `expires_at` — that is set at INSERT time (see below). Setting `expires_at = NOW()` on an already-expired row would produce a row the sweeper re-finds on every subsequent run; hard-deletion is the correct terminal state.
