...
func NewGuardrailEngine(auditor Auditor) *GuardrailEngine {
...
func (g *GuardrailEngine) auditGuardrailCheck(ctx context.Context, tenantID, userID, content string, result *GuardrailResult) (*AuditEventResult, error) {
	...
	// Build audit record using AuditEvent
	...
	// Log event using the auditor
	err := g.auditor.LogEvent(ctx, record)
