...
import (
	...
	"github.com/hondyman/uisce/backend/temporal/activities"
...
func ClientOnboardingWorkflow(ctx workflow.Context, input ClientOnboardingWorkflowInput) (*ClientOnboardingWorkflowState, error) {
	...
	err := workflow.ExecuteActivity(activityOptions, activities.ValidateClientDataActivity, ValidationInput{
		ClientID:             input.ClientID,
		VerifyKYC:            true,
	...
	err = workflow.ExecuteActivity(activityOptions, activities.RouteForAdvisorReviewActivity, RoutingInput{
		ClientID:    input.ClientID,
	...
	if timeoutOccurred {
		...
		err := workflow.ExecuteActivity(activityOptions, activities.StartTimeoutEscalationActivity, TimeoutEscalationInput{
			ClientID:     input.ClientID,
	...
	err = workflow.ExecuteActivity(activityOptions, activities.GenerateAndSendAgreementsActivity, AgreementsInput{
		ClientID:         input.ClientID,
	...
	if !signatureDone {
		...
		_ = workflow.ExecuteActivity(activityOptions, activities.SendAgreementReminderActivity, AgreementReminderInput{
			ClientID:    input.ClientID,
			ClientEmail: input.ClientEmail,
	...
	*state.AgreementsSignedTime = time.Now()
	...
	err = workflow.ExecuteActivity(activityOptions, activities.CreateAccountsAndPortfoliosActivity, AccountsInput{
		ClientID:       input.ClientID,
		RiskProfile:    validationResult.RiskProfile,
	...
	err = workflow.ExecuteActivity(activityOptions, activities.NotifyClientOnCompletionActivity, NotificationInput{
		ClientID:         input.ClientID,
		ClientEmail:      input.ClientEmail,
		ClientName:       input.ClientName,
