...
type SalesLedgerServiceInterface interface {
	List(ctx context.Context, tenantID uuid.UUID, subtypeCode string) ([]SalesLedgerRecord, error)
	Get(ctx context.Context, tenantID, id uuid.UUID) (*SalesLedgerRecord, error)
	Create(ctx context.Context, tenantID uuid.UUID, rec *SalesLedgerRecord) error
	SoftDelete(ctx context.Context, tenantID, id uuid.UUID) error
}

type Handler struct {
	svc SalesLedgerServiceInterface
}
...
func NewHandlerWithService(svc SalesLedgerServiceInterface) *Handler {
	return &Handler{svc: svc}
}

func (h *Handler) RegisterRoutes(r chi.Router) {
	r.Route("/api/master/sales-ledgers", func(r chi.Router) {
		r.Get("/", h.List)
		r.Get("/{id}", h.Get)
		r.Post("/", h.Create)
