...
type VendorServiceInterface interface {
	List(ctx context.Context, tenantID uuid.UUID, subtypeCode string) ([]VendorRecord, error)
	Get(ctx context.Context, tenantID, id uuid.UUID) (*VendorRecord, error)
	Create(ctx context.Context, tenantID uuid.UUID, rec *VendorRecord) error
	SoftDelete(ctx context.Context, tenantID, id uuid.UUID) error
}

type Handler struct {
	svc VendorServiceInterface
}
...
func NewHandlerWithService(svc VendorServiceInterface) *Handler {
	return &Handler{svc: svc}
}

func (h *Handler) RegisterRoutes(r chi.Router) {
	r.Route("/api/master/vendors", func(r chi.Router) {
		r.Get("/", h.List)
		r.Get("/{id}", h.Get)
		r.Post("/", h.Create)
