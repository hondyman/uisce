...
// getTenantContext extracts tenant_id from the request context and injects it using the
// key expected by the abbreviation service. It prefers the identity context set by
// AuthContextMiddleware, falls back to the legacy auth user context, and finally
// defaults to the gold-copy placeholder "uisce" for unauthenticated/dev requests.
func getTenantContext(ctx context.Context) context.Context {
	if tenantID, ok := identity.TenantIDFromContext(ctx); ok && tenantID != "" {
