...
func TestAIGraphSecurityInterceptor_Masking(t *testing.T) {
	dbTenant, mockTenant, err := sqlmock.New()
	require.NoError(t, err)
	defer dbTenant.Close()

	dbSystem, mockSystem, err := sqlmock.New()
	require.NoError(t, err)
	defer dbSystem.Close()

	interceptor := &AIGraphSecurityInterceptor{
		TenantPool: dbTenant,
		SystemPool: dbSystem,
	}
	...
	repo := &MockBORepository{
		BODefinitions: map[string]*BODefinition{
			"bo_orders": {
				...
				Fields: []BOField{
					{ID: "f_id", Name: "id", PhysicalColumn: "orders.id"},
					{ID: "f_ssn", Name: "social_security_number", PhysicalColumn: "orders.ssn"},
				},
			},
		},
	}
	...
	// Scenario 1: Tenant DB returns a business node with classification tags
	// In ResolveGraphGovernanceContext:
	// Try TenantPool first
	mockTenant.ExpectQuery(`SELECT.*properties.*FROM.*catalog_node`).
		WithArgs("orders.ssn").
		WillReturnRows(sqlmock.NewRows([]string{"business_properties"}).AddRow([]byte(`{"classification": "contains:pii"}`)))

	// In EvaluateEffectiveMaskingType:
	// Try TenantPool
	mockTenant.ExpectQuery(`SELECT.*masking_rules.*FROM.*abac_policies`).
		WithArgs("platform_analyst", "de305d54-75b4-431b-adb2-eb6b9e546013").
		WillReturnRows(sqlmock.NewRows([]string{"masking_rules"}).AddRow([]byte(`{"classifications": [{"tag": "contains:pii", "mask_type": "REDACT_FULL"}]}`)))

	req := SQLGenerationRequest{
		BusinessObjectID: "bo_orders",
		SelectedFields:   []string{"social_security_number"},
		TenantID:         "de305d54-75b4-431b-adb2-eb6b9e546013",
	...
	assert.Contains(t, sql, "('[REDACTED]') AS \"social_security_number\"")

	// Scenario 2: Fallback to SystemPool (Gold Copy Blueprint)
	mockTenant.ExpectQuery(`SELECT.*properties.*FROM.*catalog_node`).
		WithArgs("orders.ssn").
		WillReturnError(dbsql.ErrNoRows) // Fail on tenant local check

	mockSystem.ExpectQuery(`SELECT.*properties.*FROM.*catalog_node`).
		WithArgs("orders.ssn").
		WillReturnRows(sqlmock.NewRows([]string{"business_properties"}).AddRow([]byte(`{"classification": "contains:pii"}`)))
