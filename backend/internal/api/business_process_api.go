...
import (
	...
	"github.com/hondyman/uisce/backend/internal/business_process"
...
// BPAPIHandler handles BP API requests
type BPAPIHandler struct {
	server      *Server
	securityDeps handlers.SecurityContextDeps
}

func NewBPAPIHandler(s *Server, securityDeps handlers.SecurityContextDeps) *BPAPIHandler {
	return &BPAPIHandler{
		server:      s,
...
func (h *BPAPIHandler) APICreateBusinessProcess(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.server.DB.ExecContext(r.Context(), q, bpID, tenantID, datasourceID, templateReq.Name, templateReq.Description, bodyBytes)
	if err != nil {
	...
	logBusinessProcessAudit(h.server, r, tenantID, bpID, "created", map[string]interface{}{
...
// ============================================================================
// GET /api/bp
func (h *BPAPIHandler) APIListBusinessProcesses(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.server.DB.QueryContext(r.Context(), q, tenantID, datasourceID)
	if err != nil {
...
// ============================================================================
// GET /api/bp/:id
func (h *BPAPIHandler) APIGetBusinessProcess(w http.ResponseWriter, r *http.Request) {
	...
	err = h.server.DB.QueryRowContext(r.Context(), bpQ, id, tenantID, datasourceID).Scan(
	...
	_ = h.server.DB.QueryRowContext(r.Context(), stepsQ, id).Scan(&bp.StepCount)
...
func (h *BPAPIHandler) APIStartBusinessProcessExecution(w http.ResponseWriter, r *http.Request) {
	...
	params := business_process.DynamicWorkflowParams{
		WorkflowDefinitionID: bpID,
		BusinessObject:        obj,
	}

	we, err := h.server.TemporalClient.ExecuteWorkflow(r.Context(), workflowOptions, business_process.DynamicProcessWorkflow, params)
	if err != nil {
	...
	_, err = h.server.DB.ExecContext(r.Context(), q, instanceID, tenantID, datasourceID, bpID, req.EntityID, req.EntityType, dataJSON, we.GetID())
	if err != nil {
	...
	logBusinessProcessAudit(h.server, r, tenantID, bpID, "execution_started", map[string]interface{}{
...
func (h *BPAPIHandler) APIGetBusinessProcessInstanceStatus(w http.ResponseWriter, r *http.Request) {
	...
	err = h.server.DB.QueryRowContext(r.Context(), q, instanceID, tenantID, datasourceID).Scan(
...
func (h *BPAPIHandler) APIApproveBusinessProcessStep(w http.ResponseWriter, r *http.Request) {
	instanceID := r.URL.Query().Get("id")
	if instanceID == "" {
		instanceID = r.PathValue("id")
	}
	...
	err = h.server.DB.QueryRowContext(r.Context(), "SELECT temporal_workflow_id FROM bp_instances WHERE id = $1", instanceID).Scan(&temporalWorkflowID)
	if err != nil {
	...
	err = h.server.TemporalClient.SignalWorkflow(r.Context(), temporalWorkflowID, "", "ApprovalSignal", req.Decision)
	if err != nil {
	...
	logBusinessProcessAudit(h.server, r, tenantID, instanceID, "step_approved", map[string]interface{}{
		"instanceId":         instanceID,
...
// ============================================================================
// GET /api/bp/:id/audit
func (h *BPAPIHandler) APIGetBusinessProcessAuditTrail(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.server.DB.QueryContext(r.Context(), q, tenantID, processID, limit)
	if err != nil {
