...
func NewLayoutHandler(db *sql.DB, securityDeps handlers.SecurityContextDeps) *LayoutHandler {
	return &LayoutHandler{DB: db, securityDeps: securityDeps}
}

func (h *LayoutHandler) RegisterRoutes(r chi.Router) {
	r.Get("/layouts", h.listLayouts)
	r.Get("/layouts/{id}", h.getLayout)
	r.Post("/layouts", h.saveLayout)
...
func (h *LayoutHandler) listLayouts(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.QueryContext(r.Context(), `SELECT id, name, view, created_at, updated_at FROM public.views WHERE tenant_id = $1 AND tenant_datasource_id = $2 ORDER BY updated_at DESC`, tenantID, datasourceID)
	if err != nil {
...
func (h *LayoutHandler) getLayout(w http.ResponseWriter, r *http.Request) {
	...
	var viewRaw []byte
	var name string
	err = h.DB.QueryRowContext(r.Context(), `SELECT name, view FROM public.views WHERE id = $1 AND tenant_id = $2 AND tenant_datasource_id = $3`, id, tenantID, datasourceID).Scan(&name, &viewRaw)
	if err != nil {
...
func (h *LayoutHandler) saveLayout(w http.ResponseWriter, r *http.Request) {
	...
	if body.ID == "" {
		id := uuid.New().String()
		_, err := h.DB.ExecContext(r.Context(), `INSERT INTO public.views (id, tenant_id, tenant_datasource_id, name, view, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7)`, id, tenantID, datasourceID, body.Name, body.Layout, now, now)
		if err != nil {
	...
	_, err := h.DB.ExecContext(r.Context(), `UPDATE public.views SET name = $1, view = $2, updated_at = $3 WHERE id = $4 AND tenant_id = $5 AND tenant_datasource_id = $6`, body.Name, body.Layout, now, body.ID, tenantID, datasourceID)
	if err != nil {
...
func (h *LayoutHandler) deleteLayout(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.DB.ExecContext(r.Context(), `DELETE FROM public.views WHERE id = $1 AND tenant_id = $2 AND tenant_datasource_id = $3`, id, tenantID, datasourceID)
	if err != nil {
