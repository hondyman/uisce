...
// handleCreateValidationRule is retired - see the handler body.
func (h *validationRulesHandler) handleCreateValidationRule() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Intake closed: this endpoint's condition_json format
		// (field/operator/value payloads) has no connection to the unified
		// rule engine (internal/rules/vm.RuleNode) - and separately, no
		// code path anywhere evaluates catalog_validation_rules at all
		// (internal/validation.TriggerValidationEngine is never mounted).
		// The 233 rules already created this way were verified to target
		// entities absent from every tenant's business_objects catalog
		// (see docs/validation_rules_migration_report.json) and were
		// retired rather than migrated. New rules belong in the Monaco/
		// vm.RuleNode editor. GET/PATCH/DELETE stay open so existing rows
		// (now is_active=false) remain visible and manageable; only new
		// creation through this format is blocked.
		writeJSONError(w, http.StatusGone, "This rule-creation endpoint is retired", "endpoint_retired",
			"catalog_validation_rules intake is closed - author new validation rules in the unified rule editor (internal/rules/vm.RuleNode) instead. See docs/validation_rules_migration_report.json for why the prior format was retired rather than migrated.")
	}
}
