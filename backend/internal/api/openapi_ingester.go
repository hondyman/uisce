...
func (h *ApiDispatcherHandler) resolveGoldCopyTenant(ctx context.Context) string {
	var id string
	row := h.db.QueryRowContext(ctx, "SELECT id FROM (SELECT public.uisce_gold_copy_tenant_id() AS id) g WHERE id IS NOT NULL")
...
func (h *ApiDispatcherHandler) linkSemanticTermByName(ctx context.Context, tenantID, fieldNodeID, termName string) (string, bool) {
	...
	row := h.db.QueryRowContext(ctx, `
		SELECT id FROM catalog_node
		WHERE node_type_id = (SELECT id FROM catalog_node_types WHERE catalog_type_name = 'semantic_term' AND (tenant_id = $1::uuid OR tenant_id = public.uisce_gold_copy_tenant_id()) LIMIT 1)
	...
	row = h.db.QueryRowContext(ctx, `
		...
		WHERE edge_type_name = 'has_context'
		  AND (tenant_id = $1::uuid OR tenant_id = public.uisce_gold_copy_tenant_id())
