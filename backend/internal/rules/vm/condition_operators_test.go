...
func TestConditionOperators(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"between inclusive [low, high]", cond("n", "between", []interface{}{7, 9}), true, false},
		{"between below range", cond("i", "between", []interface{}{4.0, 9.0}), false, false},
		{"between as Value+SecondValue", RuleNode{Type: NodeTypeCondition, Condition: &RuleCondition{Field: "n", Operator: "between", Value: 1, SecondValue: "7"}}, true, false},
		{"not_between outside", cond("i", "not_between", "4,9"), true, false},
		{"not_between inside", cond("n", "not_between", []interface{}{1, 10}), false, false},
		{"between non-numeric value fails", cond("symbol", "between", []interface{}{1, 10}), false, false},
		{"between needs two bounds", cond("n", "between", []interface{}{1}), false, true},
		{"between non-numeric bound is an error", cond("n", "between", []interface{}{"a", 10}), false, true},
