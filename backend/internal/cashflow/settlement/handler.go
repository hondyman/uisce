...
type SettlementServiceInterface interface {
	List(ctx context.Context, tenantID uuid.UUID, subtypeCode string) ([]SettlementRecord, error)
	Get(ctx context.Context, tenantID, id uuid.UUID) (*SettlementRecord, error)
	Create(ctx context.Context, tenantID uuid.UUID, rec *SettlementRecord) error
...
func NewHandlerWithService(svc SettlementServiceInterface) *Handler {
	return &Handler{svc: svc}
}

func (h *Handler) RegisterRoutes(r chi.Router) {
	r.Route("/api/cash-flow/settlements", func(r chi.Router) {
		r.Get("/", h.List)
		r.Get("/{id}", h.Get)
		r.Post("/", h.Create)
