...
type AccountServiceInterface interface {
	List(ctx context.Context, tenantID uuid.UUID, subtypeCode string) ([]AccountRecord, error)
	Get(ctx context.Context, tenantID, id uuid.UUID) (*AccountRecord, error)
	Create(ctx context.Context, tenantID uuid.UUID, rec *AccountRecord) error
...
func NewHandlerWithService(svc AccountServiceInterface) *Handler {
	return &Handler{svc: svc}
}

func (h *Handler) RegisterRoutes(r chi.Router) {
	r.Route("/api/oms/accounts", func(r chi.Router) {
		r.Get("/", h.List)
		r.Get("/{id}", h.Get)
		r.Post("/", h.Create)
