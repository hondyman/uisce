...
type PositionServiceInterface interface {
	List(ctx context.Context, tenantID uuid.UUID, subtypeCode string) ([]PositionRecord, error)
	Get(ctx context.Context, tenantID, id uuid.UUID) (*PositionRecord, error)
	Create(ctx context.Context, tenantID uuid.UUID, rec *PositionRecord) error
	SoftDelete(ctx context.Context, tenantID, id uuid.UUID) error
}

type Handler struct {
	svc PositionServiceInterface
}
...
func NewHandlerWithService(svc PositionServiceInterface) *Handler {
	return &Handler{svc: svc}
}

func (h *Handler) RegisterRoutes(r chi.Router) {
	r.Route("/api/oms/positions", func(r chi.Router) {
		r.Get("/", h.List)
		r.Get("/{id}", h.Get)
		r.Post("/", h.Create)
