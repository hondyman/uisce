...
type TradeOrderServiceInterface interface {
	List(ctx context.Context, tenantID uuid.UUID, subtypeCode string) ([]TradeOrderRecord, error)
	Get(ctx context.Context, tenantID, id uuid.UUID) (*TradeOrderRecord, error)
	Create(ctx context.Context, tenantID uuid.UUID, rec *TradeOrderRecord) error
	SoftDelete(ctx context.Context, tenantID, id uuid.UUID) error
}

type Handler struct {
	svc TradeOrderServiceInterface
}
...
func NewHandlerWithService(svc TradeOrderServiceInterface) *Handler {
	return &Handler{svc: svc}
}

func (h *Handler) RegisterRoutes(r chi.Router) {
	r.Route("/api/oms/trade-orders", func(r chi.Router) {
		r.Get("/", h.List)
		r.Get("/{id}", h.Get)
		r.Post("/", h.Create)
