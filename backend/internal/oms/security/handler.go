...
type SecurityServiceInterface interface {
	List(ctx context.Context, tenantID uuid.UUID, subtypeCode string) ([]SecurityRecord, error)
	Get(ctx context.Context, tenantID, id uuid.UUID) (*SecurityRecord, error)
	Create(ctx context.Context, tenantID uuid.UUID, rec *SecurityRecord) error
	SoftDelete(ctx context.Context, tenantID, id uuid.UUID) error
}

type Handler struct {
	svc SecurityServiceInterface
}
...
func NewHandlerWithService(svc SecurityServiceInterface) *Handler {
	return &Handler{svc: svc}
}

func (h *Handler) RegisterRoutes(r chi.Router) {
	r.Route("/api/oms/securities", func(r chi.Router) {
		r.Get("/", h.List)
		r.Get("/{id}", h.Get)
		r.Post("/", h.Create)
