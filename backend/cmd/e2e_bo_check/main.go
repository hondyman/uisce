...
func main() {
	...
	if dsn == "" {
		fmt.Println("TEST_DATABASE_URL environment variable is required")
		os.Exit(1)
	}
	...
	_, err = db.ExecContext(ctx, `INSERT INTO business_objects (id, tenant_id, key, name, display_name, technical_name, created_at) VALUES ($1::uuid, $2::uuid, $3, $4, $5, $6, NOW())`, parentID, tenantID, "e2e_parent_"+parentID, "E2E Parent", "E2E Parent", "e2e_parent")
	if err != nil {
		fmt.Printf("failed to insert parent: %v\n", err)
		os.Exit(1)
	}
	_, err = db.ExecContext(ctx, `INSERT INTO business_objects (id, tenant_id, key, name, display_name, technical_name, parent_id, created_at) VALUES ($1::uuid, $2::uuid, $3, $4, $5, $6, $7::uuid, NOW())`, childID, tenantID, "e2e_child_"+childID, "E2E Child", "E2E Child", "e2e_child", parentID)
	if err != nil {
