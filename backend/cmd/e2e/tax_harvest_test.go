...
func TestTaxHarvestE2E(t *testing.T) {
	...
	resp2, err := http.Post("http://localhost:8080/api/uma/test-uma-1/tax", "application/json", strings.NewReader("{}"))
	assert.NoError(t, err)
	...
	assert.True(t, finalTaxSaved >= initialTaxSaved+100000, "Tax savings should increase by at least $100K")
}
