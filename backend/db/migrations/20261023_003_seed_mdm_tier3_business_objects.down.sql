-- Reverses 20261023_003_seed_mdm_tier3_business_objects.up.sql.
-- Removes only the 3 tier-3 MDM BOs (by bo_key) in the gold-copy tenant, and the
...
BEGIN
    ...
    WHERE tenant_id = v_tenant AND bo_key IN ('xref', 'entity_relationship', 'attribute_value') AND is_core = true;
