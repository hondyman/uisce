-- 20261023_003_seed_mdm_tier3_business_objects.up.sql
--
-- Tier 3 MDM business objects: xref, entity_relationship and attribute_value, the three tables
-- that reference other records polymorphically (entity_type + entity_id).
--
-- Depends on tiers 1 and 2 (source_system, attribute_def). Same conventions as those: gold-copy
-- tenant, is_core, model_id = id, ids resolved from catalog_node by qualified_path, idempotent,
-- skips with a NOTICE when the gold-copy tenant or the /mdm catalog nodes are absent.
--
-- Relationships: only declared FKs (attribute_value -> attribute_def, xref -> source_system).
-- The polymorphic references (xref.entity_id, entity_relationship.source/target_entity_id,
-- attribute_value.entity_id, each qualified by an *_entity_type column) cannot be expressed as a
-- join basis, so they are NOT modelled as relationships here; they are described in each BO's
-- description. Typed polymorphic relationships need a discriminator in
-- business_object_relationships and are a separate change.
--
-- Business keys: xref.external_id; the other two have no natural key and use id. All three are
-- VALID_TIME (xref valid_from/valid_to, entity_relationship effective_date/end_date,
-- attribute_value effective_from/effective_to).
...
BEGIN
    ...
    IF v_tenant IS NULL THEN
        RAISE NOTICE 'mdm tier-3 seed: no gold-copy tenant, skipping';
        RETURN;
    END IF;

    SELECT tenant_datasource_id INTO v_ds
    FROM public.catalog_node
    WHERE tenant_id = v_tenant AND qualified_path = '/mdm/xref'
    LIMIT 1;
    IF v_ds IS NULL THEN
        RAISE NOTICE 'mdm tier-3 seed: /mdm/xref not cataloged, skipping';
        RETURN;
    END IF;
    ...
    INSERT INTO _mdm_bo VALUES
      ('xref', 'Cross Reference', 'BRIDGE', 'xref', 'external_id', 'Maps a master record (entity_type + entity_id) to its identifier in a source system.', 'VALID_TIME', 'valid_from', 'valid_to'),
      ('entity_relationship', 'Entity Relationship', 'BRIDGE', 'entity_relationship', 'id', 'Typed, dated relationship between two master records (source and target entity_type + entity_id).', 'VALID_TIME', 'effective_date', 'end_date'),
      ('attribute_value', 'Attribute Value', 'ENTITY', 'attribute_value', 'id', 'Value of a custom (EAV) attribute on a master record (entity_id), typed by attribute_def.', 'VALID_TIME', 'effective_from', 'effective_to');
    ...
    FOR v_bo IN SELECT * FROM _mdm_bo LOOP
        ...
        IF v_table IS NULL THEN
            RAISE NOTICE 'mdm tier-3 seed: table node /mdm/% missing, skipping %', v_bo.tbl, v_bo.bo_key;
            CONTINUE;
        END IF;
        ...
        RAISE NOTICE 'mdm tier-3 seed: % -> % fields inserted', v_bo.bo_key, v_n;
    END LOOP;
    ...
    FOR v_rel IN
        SELECT * FROM (VALUES
          ('attribute_def', 'attribute_value', 'attribute_def_id', 'values',      'attribute_def'),
          ('source_system', 'xref',            'source_system_id', 'xrefs',       'source_system')
