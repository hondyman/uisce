...
-- Migration: Seed Semantic Layer for customers (Northwinds)
-- Description: Auto-generates Semantic Terms and mappings for the customers table.

DO $$
DECLARE
    customers_table_id UUID;
    column_rec RECORD;
    term_type_id UUID;
    column_type_id UUID;
...
BEGIN
    ...
    -- 3. Get customers Table Node (Northwinds)
    -- We alias catalog_node as cn to avoid ambiguous column reference for tenant_id
    SELECT cn.id, cn.tenant_id, cn.tenant_datasource_id INTO customers_table_id, tenant_id, datasource_id
    FROM catalog_node cn
    WHERE cn.node_name = 'customers' 
      AND cn.node_type_id IN (SELECT id FROM catalog_node_type WHERE catalog_type_name = 'table')
    LIMIT 1;

    IF customers_table_id IS NULL THEN
        RAISE NOTICE 'customers table not found. Skipping seeding.';
        RETURN;
    END IF;

    RAISE NOTICE 'Seeding terms for table: customers (%)', customers_table_id;

    -- 4. Iterate over columns that do NOT have mappings yet
    FOR column_rec IN 
        SELECT c.id, c.node_name, c.node_type_id
        FROM catalog_node c
        WHERE c.parent_id = customers_table_id
    ...
    LOOP
        ...
        INSERT INTO catalog_node (
            ...
            new_term_id, 
            column_rec.node_name,
            term_type_id,
            tenant_id,
            datasource_id,
            jsonb_build_object(
                'title', INITCAP(REPLACE(column_rec.node_name, '_', ' ')), 
                'data_type', 'string', -- Default to string
                'auto_generated', true
            ),
            column_rec.node_name, -- Use node_name also as qualified_path
            NOW(), NOW()
        );
    ...
    RAISE NOTICE 'Seeding complete for customers.';
END $$;
