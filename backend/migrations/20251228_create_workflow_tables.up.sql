...
ALTER TABLE workflow_definitions ADD COLUMN IF NOT EXISTS active_version_id UUID;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_active_version') THEN
...
CREATE INDEX IF NOT EXISTS idx_workflow_versions_definition_id ON workflow_versions(definition_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_workflow_versions_published ON workflow_versions(definition_id) WHERE is_published = TRUE;
