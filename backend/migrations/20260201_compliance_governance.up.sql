...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'catalog_node' AND column_name = 'kind') THEN
...
CREATE INDEX IF NOT EXISTS idx_compliance_events_type ON semantic_compliance_events(event_type);
CREATE INDEX IF NOT EXISTS idx_compliance_events_created ON semantic_compliance_events(created_at DESC);
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'scheduler_jobs') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduler_jobs' AND column_name = 'compliance') THEN
            ...
            CREATE INDEX IF NOT EXISTS idx_scheduler_jobs_compliance ON scheduler_jobs USING GIN (compliance);
        END IF;
    ELSE
