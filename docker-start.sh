...
start_backend() {
    ...
    BACKEND_SERVICES=(backend fabric-builder wealth-management ai-builder semantic-engine governance validation-service rule-engine-service notifications-service policy-service search-service event-router)
