...
export const QueryMutationTimeline: React.FC<QueryMutationTimelineProps> = ({ messages }) => {
  ...
  const getIntentConfig = (intent?: MutationIntent) => {
    switch (intent) {
      case 'new_query':
        return { icon: <Database size={14} />, color: '#38BDF8', label: 'Generated New Query' };
      case 'drill_down':
        return { icon: <ArrowDownCircle size={14} />, color: '#A78BFA', label: 'Drilled Down' };
      case 'drill_across':
        return { icon: <Repeat size={14} />, color: '#F472B6', label: 'Swapped Dimension' };
      case 'add_filter':
        return { icon: <Filter size={14} />, color: '#2DD4BF', label: 'Applied Filter' };
      case 'add_measure':
        return { icon: <PlusCircle size={14} />, color: '#10B981', label: 'Added Metric' };
      case 'remove_element':
        return { icon: <MinusCircle size={14} />, color: '#FB923C', label: 'Removed Element' };
      default:
        return { icon: <Sparkles size={14} />, color: '#94A3B8', label: 'Updated View' };
    }
  };
          ...
          >
            ...
            {isUser && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, maxWidth: '85%' }}>
                ...
                <MessageSquare size={15} color={isDark ? '#64748B' : '#94A3B8'} />
              </Box>
            )}
