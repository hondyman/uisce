...
export default function ScanProgressModal({ 
  ...
  useEffect(() => {
    ...
    connectionTimeout = setTimeout(() => {
      if (!hasReceivedData && isStreaming) {
        console.warn('[SSE] Connection timeout - no data received within ' + (CONNECTION_TIMEOUT / 1000) + ' seconds');
        eventSource.close();
        
        if (connectionAttempts < MAX_RETRIES) {
          console.log(`[SSE] Retrying connection (${connectionAttempts + 1}/${MAX_RETRIES})...`);
          setConnectionAttempts(prev => prev + 1);
          setIsStreaming(false);
        } else {
          setStreamError('The scan service is not responding. Please verify the backend is running and try again.');
          setIsStreaming(false);
        }
      }
    }, CONNECTION_TIMEOUT);
    ...
    eventSource.onerror = (e) => {
      ...
      // Retry logic
      if (connectionAttempts < MAX_RETRIES) {
        console.log(`[SSE] Connection failed, retrying (${connectionAttempts + 1}/${MAX_RETRIES})...`);
        setIsStreaming(false);
  ...
  return (
    <Dialog open={open} onClose={loading || isStreaming ? undefined : onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent>
        ...
        {useStreaming && isStreaming && !progress && !streamError && (
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 4 }}>
            ...
            {connectionAttempts > 0 && (
              <Typography variant="caption" color="text.secondary" sx={{ mt: 1 }}>
                Connection attempt {connectionAttempts + 1} of {MAX_RETRIES + 1}
              </Typography>
            )}
          </Box>
        )}
          ...
          <Alert severity="error" sx={{ mt: 2, mb: 2 }}>
            ...
            {connectionAttempts > 0 && (
              <Typography variant="caption" sx={{ display: 'block', mt: 1 }}>
                Attempts: {connectionAttempts + 1}/{MAX_RETRIES + 1}
              </Typography>
            )}
          </Alert>
        )}
