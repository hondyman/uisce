...
// IRRCurveChart and JCurvePlot not used in the mobile FoF dashboard variant
...
// CapitalDeploymentChart not used in mobile FoF dashboard
import { PortfolioCompositionChart } from './components/PortfolioCompositionChart';
import { FundOfFundsAllocationChart } from './components/FundOfFundsAllocationChart';
import { ManagerPerformanceHeatmap } from './components/ManagerPerformanceHeatmap';
...
interface MobileResponsiveFoFPrivateMarketsDashboardProps {
  userId?: string;
  realTimeData?: { [fundId: string]: { metrics: any; lastUpdate: string } };
}

export const MobileResponsiveFoFPrivateMarketsDashboard: React.FC<MobileResponsiveFoFPrivateMarketsDashboardProps> = ({
  realTimeData = {}
}) => {
  ...
  // Key metrics cards for mobile - FoF focused
  const renderKeyMetrics = () => (
    <Grid container spacing={isMobile ? 1 : 2} sx={{ mb: isMobile ? 2 : 3 }}>
      {[
        { title: 'Total AUM', value: '$15.6B', change: '+8.7%', icon: <AccountBalance />, color: 'primary' },
        { title: 'Net IRR', value: '14.2%', change: '+2.8%', icon: <TrendingUp />, color: 'success' },
