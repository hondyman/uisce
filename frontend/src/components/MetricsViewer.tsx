...
import { getAllMetrics, getDomains, getFunctionMappings, Metric, FunctionMapping } from '../lib/calculationLibrary';
...
const MetricsViewer: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const [domainsData, metricsData, mappingsData] = await Promise.all([
          getDomains(),
          getAllMetrics(),
          getFunctionMappings()
        ]);
        ...
        setFunctionMappings(mappingsData);
      } catch (error) {
  ...
  const filteredMetrics = useMemo(() => {
    ...
    let metrics = selectedDomain === 'all' ? allMetrics : allMetrics.filter(metric => metric.domain === selectedDomain);
    ...
    return metrics;
  }, [selectedDomain, searchTerm, allMetrics, loading]);
