...
import type { BORelationship } from '../../studio-core/binding/boRelationships';
import { fkColumnFromJoinCondition } from '../../studio-core/binding/boRelationships';
import { ensureRelatedDataSource } from '../../studio-core/binding/ensureRelatedDataSource';
...
const PropertiesPanel: React.FC<PropertiesPanelProps> = ({ selectedId, onSelectComponent, draft, setDraft, layout, onLayoutChange, tenantId }) => {
  ...
  const primarySource = boSources[0];
  const primaryCfg = primarySource?.config as unknown as BusinessObjectDataSourceConfig | undefined;
  const [pageRelatedBOs, setPageRelatedBOs] = useState<BORelationship[]>([]);
  useEffect(() => {
    if (!primaryCfg?.boId) {
      setPageRelatedBOs([]);
      return;
    ...
    apiClient<{ relatedObjects?: BORelationship[] }>(`/business-objects/${primaryCfg.boId}/relationships`, {
      ...
      .then((data) => {
        if (!cancelled) setPageRelatedBOs((data?.relatedObjects || []).filter((r) => r.kind !== 'relatedTable'));
      })
      .catch(() => { if (!cancelled) setPageRelatedBOs([]); });
    return () => { cancelled = true; };
  ...
  if (fieldSelection && component && isFormLikeWidget(component.type)) {
    ...
    return (
      <Paper elevation={0} sx={{ height: '100%', bgcolor: 'background.paper', overflowY: 'auto' }}>
        ...
        <Box sx={{ mt: 2 }}>
          <Typography variant="overline" color="text.secondary" sx={{ fontSize: 10, fontWeight: 700 }}>Field events</Typography>
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>
            FieldChange / FieldEdit format this field only. They do not save the record.
          </Typography>
          <Button
            size="small"
            variant="outlined"
            onClick={() => {
              const rule = newPresentationRule('fieldChange', {
                label: `${fieldMeta?.displayName || fieldName} change`,
                source: { kind: 'field', id: componentId, termKey: fieldName },
                actions: [{ target: { kind: 'field', id: componentId, fieldName } }],
              });
              setDraft((prev) => ({ ...prev, presentationEvents: [...(prev.presentationEvents || []), rule] }));
            }}
          >
            Add FieldChange
          </Button>
  ...
  return (
    <Paper elevation={0} sx={{ height: '100%', bgcolor: 'background.paper', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        ...
        <Tab label="Events" sx={{ minHeight: 36, py: 0.5, fontSize: 12 }} />
      </Tabs>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Stack spacing={1.5} sx={{ mt: 1 }}>
            ...
            >
              ...
              <MenuItem value="command">Start a FIX / OMS command</MenuItem>
            </TextField>
            {((component.props?.action as string) || 'navigate') !== 'openUrl' && (component.props?.action as string) !== 'command' ? (
              <TextField
            ...
            {(component.props?.action as string) === 'command' && (
              <>
                <TextField
                  select fullWidth size="small" label="Command"
                  value={String(component.props?.command || 'NewOrderSingle').replace(/^fix\./, '')}
                  onChange={(e) => updateProps({ command: e.target.value })}
                  helperText="Starts FIXOrderEntryWorkflow. Does not save this page or write fills."
                >
                  <MenuItem value="NewOrderSingle">NewOrderSingle</MenuItem>
                  <MenuItem value="CancelReplace">CancelReplace</MenuItem>
                  <MenuItem value="Cancel">Cancel</MenuItem>
                </TextField>
                <TextField
                  fullWidth size="small" label="FIX session id (optional)"
                  placeholder="FIX.4.4:UISCE->DEMOAGENT"
                  value={(component.props?.sessionId as string) || ''}
                  onChange={(e) => updateProps({ sessionId: e.target.value })}
                />
              </>
            )}
          </Stack>
      ...
      {component?.type === 'FixCommand' && (
        <Box sx={{ mb: 3 }}>
          <Typography variant="overline" color="text.secondary" fontWeight="bold">FIX command</Typography>
          <Stack spacing={1.5} sx={{ mt: 1 }}>
            <TextField
              fullWidth size="small" label="Label"
              value={(component.props?.label as string) || ''}
            ...
            <TextField
              ...
              value={String(component.props?.command || 'NewOrderSingle').replace(/^fix\./, '')}
              onChange={(e) => updateProps({ command: e.target.value })}
              helperText="Gold-copy core owns the default. Tenants extend via overlay, never by editing core widgets."
            >
              <MenuItem value="NewOrderSingle">NewOrderSingle</MenuItem>
              <MenuItem value="CancelReplace">CancelReplace</MenuItem>
              <MenuItem value="Cancel">Cancel</MenuItem>
      ...
      {component && FIELD_BOUND_TYPES.includes(component.type) && primarySource && (
        <Box sx={{ mb: 3 }}>
          <Typography variant="overline" color="text.secondary" fontWeight="bold">Business Object</Typography>
          <TextField
            select
            size="small"
            fullWidth
            sx={{ mt: 1 }}
            label="Bind to"
            value={tableCfg?.boId || primaryCfg?.boId || ''}
            helperText={(() => {
              const rel = pageRelatedBOs.find((r) => r.targetObjectId === tableCfg?.boId);
              return rel ? `${rel.cardinality} · ${rel.joinCondition}` : 'Primary Business Object, or a related one from the graph.';
            })()}
            onChange={(e) => {
              const boId = e.target.value;
              void (async () => {
                if (boId === primaryCfg?.boId) {
                  updateProps({ dataSourceId: primarySource.id });
                  return;
                }
                const rel = pageRelatedBOs.find((r) => r.targetObjectId === boId);
                if (!rel) return;
                const result = await ensureRelatedDataSource(draft.dataSources || [], {
                  parentSourceId: primarySource.id,
                  parentBoId: primaryCfg!.boId,
                ...
                if (!result) return;
                setDraft((prev) => ({ ...prev, dataSources: result.sources }));
                updateProps({ dataSourceId: result.sourceId });
              })();
            }}
          >
            <MenuItem value={primaryCfg?.boId || ''}>{primaryCfg?.displayName || primaryCfg?.boKey || 'Primary'}</MenuItem>
            {pageRelatedBOs.map((r) => (
              <MenuItem key={r.targetObjectId} value={r.targetObjectId}>
                {r.relatedObjectName} ({r.cardinality})
              </MenuItem>
            ))}
          </TextField>
        </Box>
      ...
      {panelTab === 3 && componentId && (
        <Box>
          <Typography variant="overline" color="text.secondary" fontWeight="bold">Formatting events</Typography>
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1.5 }}>
            Hide, restyle, or relabel this {layoutNode ? 'section' : 'widget'} when an expression is true. Does not save or validate data.
          </Typography>
          {(draft.presentationEvents || [])
            .filter((r) => r.actions.some((a) => a.target.id === componentId))
            .map((rule) => (
              <RuleCard
                key={rule.id}
                rule={rule}
                boKey={boKey}
                targets={collectPresentationTargets(draft)}
                onChange={(patch) => setDraft((prev) => ({
                  ...prev,
                  presentationEvents: (prev.presentationEvents || []).map((r) => (r.id === rule.id ? { ...r, ...patch } : r)),
                }))}
                onRemove={() => setDraft((prev) => ({
                  ...prev,
                  presentationEvents: (prev.presentationEvents || []).filter((r) => r.id !== rule.id),
                }))}
              />
            ))}
          <Button
            ...
            onClick={() => {
              const rule = newPresentationRule('fieldChange', {
                label: `${component?.label || layoutNode?.type || componentId}`,
                actions: [{ target: { kind: layoutNode ? 'section' : 'widget', id: componentId } }],
              });
              setDraft((prev) => ({ ...prev, presentationEvents: [...(prev.presentationEvents || []), rule] }));
            }}
          >
            Add FieldChange rule
          </Button>
        </Box>
      )}
      </Box>
