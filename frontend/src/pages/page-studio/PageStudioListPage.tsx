...
const PageStudioListPage: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    setAiError(null);
    try {
      const spec = await PageStudioApi.generateSpec(bo.id, bo.key, bo.display_name || bo.name, aiDescription);
      ...
      <Dialog open={aiOpen} onClose={() => !aiGenerating && setAiOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Pick a Business Object and describe what you want. The page's widgets pull real data from that object automatically - the description only guides which widgets (KPI summary, trend chart, table) get picked.
          </Typography>
