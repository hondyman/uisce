...
/** An empty container's drop target - accepts a new widget dragged from ComponentPalette. */
const EmptySlot: React.FC<{ nodeId: string }> = ({ nodeId }) => {
        ...
        const body = (
            ...
            >
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', flexDirection: node.type === 'Row' ? 'row' : 'column', gap: 2, ...(node.style as React.CSSProperties | undefined) }}>
                    ...
                    {(!node.children || node.children.length === 0) && <EmptySlot nodeId={nodeId} />}
                </Box>
            </Box>
        );
