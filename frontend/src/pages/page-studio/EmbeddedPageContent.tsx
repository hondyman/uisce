...
const EmbeddedPageContent: React.FC<EmbeddedPageContentProps> = ({ slug, tenantId, recordId }) => {
  ...
  const wrapped = (
    <PresentationProvider rules={page.presentationEvents || []}>{body}</PresentationProvider>
  );
  return recordId
    ? <SelectionProvider key={`${page.id || slug}:${recordId}`} initialSelection={initialSelection}>{wrapped}</SelectionProvider>
    : wrapped;
};
