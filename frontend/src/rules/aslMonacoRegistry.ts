...
export async function registerUisceExpressionLanguage(monaco: typeof Monaco): Promise<void> {
  ...
  monaco.languages.registerCompletionItemProvider(UISCE_EXPRESSION_LANGUAGE, {
    ...
    provideCompletionItems(model, position) {
      ...
      const toFieldItem = (f: AslFieldMeta, detail: string = 'semantic term'): Monaco.languages.CompletionItem => ({
        label: { label: f.name, description: f.type },
        kind: monaco.languages.CompletionItemKind.Field,
        detail,
        documentation: f.description
          ? { value: `**${f.name}**  \`${f.type}\`  ${detail}\n\n${f.description}` }
          : { value: `**${f.name}**  \`${f.type}\`  ${detail}` },
        insertText: f.name,
        sortText: '0_' + f.name,
        range,
      ...
      // For cross-entity contexts (client.*), use the field list filtered
      // by that entity; for bare identifiers at top level (unqualified by
      // a dot), fall through to show both fields and fns.
