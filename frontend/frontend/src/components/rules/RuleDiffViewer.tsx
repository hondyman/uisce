        ...
        {loading ? (
          ...
          <div className="mt-4">
            ...
            <Dialog open={showDslDiff} onClose={() => setShowDslDiff(false)} fullWidth maxWidth="xl">
              ...
              <DialogContent>
                {/* Monaco Diff Editor - we guard usage; tests mock this */}
                {/* eslint-disable-next-line @typescript-eslint/ban-ts-comment */}
                {/* @ts-ignore */}
                <div style={{ height: 600 }}>
                  {/* Importing DiffEditor dynamically to avoid SSR issues */}
                  {/* Note: @monaco-editor/react is mocked in Vitest */}
                  {/* @ts-ignore */}
                  <div style={{ height: 600 }}>
                    {/* Safe dynamic import */}
                    {/* We render a fallback if DiffEditor is not available */}
                    {typeof (require as any) === 'function' ? (
                      // @ts-ignore
                      require('@monaco-editor/react').DiffEditor ? (
                        // @ts-ignore
                        React.createElement(require('@monaco-editor/react').DiffEditor, {
                          original: diff.base?.expression || diff.base?.script || '',
                          modified: diff.current?.expression || diff.current?.script || '',
                          language: 'semrule',
                          options: { readOnly: true, minimap: { enabled: false } }
                        })
                      ) : (
                        <Typography variant="body2">Diff editor unavailable in this environment.</Typography>
                      )
                    ) : (
                      <Typography variant="body2">Diff editor unavailable in this environment.</Typography>
                    )}
