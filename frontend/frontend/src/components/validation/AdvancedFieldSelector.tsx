...
interface AdvancedFieldSelectorProps {
  ...
  // Optional set of highlighted full paths in dot notation (e.g., "customer.name")
...
const AdvancedFieldSelector: React.FC<AdvancedFieldSelectorProps> = ({
  ...
  // Helper to determine whether a field full path is highlighted
  const isFieldHighlighted = (fieldName: string) => {
    if (!highlightedFields || !selectedEntity) return false;
    const fullPath = `${selectedEntity}${fieldPath.length ? '.' + fieldPath.join('.') : ''}.${fieldName}`;
    return highlightedFields.has(fullPath);
  };
              ...
              <List>
                {filteredFields.map(field => (
                  <ListItem
                    ...
                    disablePadding
                  >
                    <ListItemButton onClick={() => handleSelectField(field)} sx={isFieldHighlighted(field.name) ? { backgroundColor: (theme) => 'rgba(255,215,0,0.15)', borderLeft: '3px solid #f5c518' } : {}}>
                      <ListItemIcon>
                        <CallReceivedIcon />
                      </ListItemIcon>
                      <ListItemText
