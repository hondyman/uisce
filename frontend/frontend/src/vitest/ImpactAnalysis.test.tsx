...
test('calls onFieldsDetected callback with full field paths', async () => {
  const mockImpact = {
    rule_id: 'r1',
    fields: [ { semantic_term_id: '', bo_field_id: 'f1', business_object_id: 'b1', field_path: ['customer', 'name'] } ],
    semantic_terms: [],
  ...
  const onFieldsDetected = vi.fn();

  render(<ImpactAnalysis ruleID={'r1'} rule={{ target_entity: 'business_object', field_name: 'name', rule_condition: '', severity: 'error' }} tenantId={'t1'} onFieldsDetected={onFieldsDetected} />);
  ...
  expect(onFieldsDetected).toHaveBeenCalledWith(['b1.customer.name']);
});
